...
OUT_DIR = Path("data/er/blocking_pairs_shards")  # per-shard files


def make_pairs_shard_asset(i: int):

  @asset(name=f"er_pairs_shard_{i}", deps=["company_blocks_adaptive"])
  def er_pairs_shard_i(context: AssetExecutionContext,
    ...
    with duckdb.get_connection() as con:
      ...
      con.execute(f"""
            COPY (SELECT * FROM er.blocking_pairs_{i})
            TO '{out_path.as_posix()}'
            (FORMAT PARQUET, OVERWRITE_OR_IGNORE TRUE);
        """)
      cnt = con.execute(f"SELECT COUNT(*) FROM er.blocking_pairs_{i}").fetchone()[0]

    return MaterializeResult(
      metadata={
        "shard_index": MetadataValue.int(i),
        "pairs_in_shard": MetadataValue.int(cnt),
    ...
    #     "shard_index": MetadataValue.int(i),
    #     "pairs_in_shard": MetadataValue.int(cnt),
    #     "blocks_source": MetadataValue.text("er.blocks_adaptive"),
    #   })

  return er_pairs_shard_i


# Build all shard assets
pair_shard_assets: List = [make_pairs_shard_asset(i) for i in range(SHARD_MODULUS)]


@asset(deps=["pair_shard_assets"], name="er_blocking_pairs")
def er_blocking_pairs_union(context: AssetExecutionContext, duckdb: DuckDBResource):
  ...
  with duckdb.get_connection() as con:
    ...
    con.execute("""
            ...
            SELECT * FROM read_parquet('data/er/blocking_pairs_shards/shard=*.parquet');
        """)
